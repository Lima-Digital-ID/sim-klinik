...
class Pendaftaran extends CI_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data_pendaftaran = array(
                ...
                'tipe_periksa' => 1,
                'id_tujuan_periksa'     =>  $this->input->post('tujuan_periksa'),
            );
            ...
            $this->data['tujuan'] = $this->Pendaftaran_model->get_all_tujuan();
    ...
    function ubah_status_dokter($id_dokter)
    {
        ...
        if ($dokter->is_jaga == 1) {
            $this->Tbl_dokter_model->update($id_dokter, array(
                "is_jaga" => 0, "dtm_upd" => date("Y-m-d H:i:s",  time()),
                "no_pendaftaran" => null
            ));
            //Set session sukses
            ...
            $this->Tbl_dokter_model->update($id_dokter, array(
                "is_jaga" => 1, "dtm_upd" => date("Y-m-d H:i:s",  time())
            ));
    ...
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('tujuan_periksa', 'Tujuan Pasien', 'trim|required');
    ...
    public function json_detail_pasien($id)
    {
        header('Content-Type: application/json');
        echo json_encode($this->Tbl_pasien_model->get_by_id($id));
    }
