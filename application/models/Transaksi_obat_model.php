...
class Transaksi_obat_model extends CI_Model
{
    ...
    function json($id_klinik)
    {
        ...
        if ($_GET['dari'] && $_GET['sampai']) {
            // $this->datatables->where('tbl_purchases.tgl_po', $_GET['supplier']);
            $this->db->where("tbl_purchases.tanggal_po BETWEEN '" . $_GET['dari'] . "' AND '" . $_GET['sampai'] . "'");
        }
        if ($_GET['jenis_bayar']) {
            $this->datatables->where('jenis_pembayaran', $_GET['jenis_bayar'] == 'cash' ? 0 : 1);
        }
